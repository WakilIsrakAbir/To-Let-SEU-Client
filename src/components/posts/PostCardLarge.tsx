...
export default function PostCardLarge({ post }: PostCardLargeProps) {
  ...
  return (
    ...
    >
      ...
      {/* 3. Location & "See More" Side-by-Side in the Same Row */}
      <div className="flex items-center justify-between gap-2">
        {/* Location Badge (Left) */}
      ...
      {/* 5. Rent Bar (Positioned directly below Image for optimal conversion & zero text overflow) */}
      <div className="px-3.5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500/10 via-teal-500/5 to-slate-50 dark:from-emerald-950/35 dark:via-slate-800/60 dark:to-slate-900 border border-emerald-500/20 dark:border-emerald-500/20 flex items-center justify-between gap-2 shadow-xs flex-wrap">
        {/* Left: Rent Amount */}
        ...
        {/* Right: Badges (Negotiable & Bills Status - cleanly wrapped without overflow) */}
        <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
          {post.rentType === 'negotiable' && (
            <span className="px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wide bg-violet-100 dark:bg-violet-950/60 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-800">
              Negotiable
            </span>
          )}
          <span
            className={`px-2 py-0.5 rounded-md text-[11px] font-bold border flex items-center gap-1 whitespace-nowrap ${
